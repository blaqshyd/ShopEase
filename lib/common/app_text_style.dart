import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

class AppTextStyle {
  AppTextStyle._();

//! h1

  static TextStyle h1Regular = GoogleFonts.inter(
    fontSize: 32,
    color: Colors.black,
  );
  static TextStyle h1Medium = GoogleFonts.inter(
    fontSize: 32,
    color: Colors.black,
    fontWeight: FontWeight.w500,
  );
  static TextStyle h1Bold = GoogleFonts.inter(
    fontSize: 32,
    color: Colors.black,
    fontWeight: FontWeight.bold,
  );

//! h2 - 28

  static TextStyle h2Regular = GoogleFonts.inter(
    fontSize: 28,
    color: Colors.black,
  );
  static TextStyle h2Medium = GoogleFonts.inter(
    fontSize: 28,
    color: Colors.black,
    fontWeight: FontWeight.w500,
  );
  static TextStyle h2Bold = GoogleFonts.inter(
    fontSize: 28,
    color: Colors.black,
    fontWeight: FontWeight.bold,
  );

//! h3

  static TextStyle h3Regular = GoogleFonts.inter(
    fontSize: 24,
    color: Colors.black,
  );
  static TextStyle h3Medium = GoogleFonts.inter(
    fontSize: 24,
    color: Colors.black,
    fontWeight: FontWeight.w500,
  );
  static TextStyle h3Bold = GoogleFonts.inter(
    fontSize: 24,
    color: Colors.black,
    fontWeight: FontWeight.bold,
  );

//! h4

  static TextStyle h4Regular = GoogleFonts.inter(
    fontSize: 20,
    color: Colors.black,
  );
  static TextStyle h4Medium = GoogleFonts.inter(
    fontSize: 20,
    color: Colors.black,
    fontWeight: FontWeight.w500,
  );
  static TextStyle h4Bold = GoogleFonts.inter(
    fontSize: 20,
    color: Colors.black,
    fontWeight: FontWeight.w700,
  );

//! h5
  static TextStyle h5Regular = GoogleFonts.inter(
    fontSize: 18,
    color: Colors.black,
  );
  static TextStyle h5Medium = GoogleFonts.inter(
    fontSize: 18,
    color: Colors.black,
    fontWeight: FontWeight.w500,
  );
  static TextStyle h5Bold = GoogleFonts.inter(
    fontSize: 18,
    color: Colors.black,
    fontWeight: FontWeight.bold,
  );

  //! body

  static TextStyle b1Medium = GoogleFonts.inter(
    fontSize: 16,
    color: Colors.black,
    fontWeight: FontWeight.w500,
  );
  static TextStyle b1Bold = GoogleFonts.inter(
    fontSize: 16,
    color: Colors.black,
    fontWeight: FontWeight.bold,
  );
  static TextStyle b1Regular = GoogleFonts.inter(
    fontSize: 16,
    color: Colors.black,
  );

  //! body2

  static TextStyle b2Medium = GoogleFonts.inter(
    fontSize: 14,
    color: Colors.black,
    fontWeight: FontWeight.w500,
  );
  static TextStyle b2Bold = GoogleFonts.inter(
    fontSize: 14,
    color: Colors.black,
    fontWeight: FontWeight.bold,
  );
  static TextStyle b2Regular = GoogleFonts.inter(
    fontSize: 14,
    color: Colors.black,
  );

//! caption

  static TextStyle captionRegular = GoogleFonts.inter(
    fontSize: 12,
    color: Colors.black,
  );
  static TextStyle captionMedium = GoogleFonts.inter(
    fontSize: 12,
    color: Colors.black,
    fontWeight: FontWeight.w500,
  );
  static TextStyle captionBold = GoogleFonts.inter(
    fontSize: 12,
    color: Colors.black,
    fontWeight: FontWeight.bold,
  );
}
